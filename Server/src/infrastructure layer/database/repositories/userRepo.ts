...
import { ICartItem, IOrder } from "../../../domain layer/entities/order";
import { IEnrolledCourse, IUser } from "../../../domain layer/entities/User";
...
export interface IUserRepository {
  ...
  removeEnrolledBundle(userId: string, bundleId: string): Promise<void>;
  removeEnrolledCoursesFromBundle(
    userId: string,
    bundleId: string,
    courseIds: string[]
  ): Promise<void>;
...
export class UserRepository implements IUserRepository {
  ...
  async clearCart(userId: string) {
    await UserModel.findByIdAndUpdate(userId, { cart: [], cartTotal: 0 });
  }

  async removeEnrolledBundle(userId: string, bundleId: string): Promise<void> {
    await UserModel.updateOne(
      { _id: userId },
      { $pull: { enrolledBundles: { bundleId } } }
    );
  }
  async removeEnrolledCoursesFromBundle(
    userId: string,
    bundleId: string,
    courseIds: string[]
  ): Promise<void> {
