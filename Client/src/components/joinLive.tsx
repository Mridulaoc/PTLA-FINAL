...
import { fetchClass } from "../store/features/classSlice";
...
import { setError } from "../store/features/webRTCSlice";
...
const Join = () => {
  ...
  useEffect(() => {
    ...
    console.log(classSchedule.nextOccurrence);
    const checkTime = () => {
      const now = new Date();
      const classStartTime = new Date(classSchedule.nextOccurrence!);
      const classEndTime = new Date(classStartTime.getTime() + 60000);
      console.log(
